...
BEGIN
    ...
    ;WITH Postings AS (
        ...
        WHERE p.PostedAt <= @AsOfDate AND (@IncludeVoided = 1 OR p.VoidedAt IS NULL)
    ),
    Rollup AS (
        SELECT
            LedgerId,
    ...
    SELECT
        ...
        r.Total,
        CASE
            WHEN r.Total < 0 THEN 'CREDIT' /* inline in a CASE */
            WHEN r.Total = 0 THEN 'ZERO'
            ELSE 'DEBIT'
        END AS Sign
    FROM dbo.Ledger AS l
    INNER JOIN Rollup AS r ON r.LedgerId = l.LedgerId
    LEFT JOIN dbo.LedgerOverride AS o ON o.LedgerId = l.LedgerId AND o.EffectiveFrom <= @AsOfDate -- multi-predicate join
    ORDER BY l.Name;
