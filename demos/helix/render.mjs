...
if (check) {
  ...
  if (!existsSync(recording)) {
    fail(`no recording at ${recording}. Run "npm run helix".`);
  }
  ...
  console.log(`helix demo: recording matches current formatter output, all ${actual.toFixed(1)}s of it`);
  process.exit(0);
}
...
try {
  ...
  console.log(`helix demo: ${actual.toFixed(1)}s recorded, against ${expected.toFixed(1)}s of tape`);
} finally {
