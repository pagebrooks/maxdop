...
if (check) {
  ...
  if (!existsSync(recording)) {
    fail(`no recording at ${recording}. Run "npm run nvim".`);
  }
  ...
  console.log(`nvim demo: recording matches current formatter output, all ${actual.toFixed(1)}s of it`);
  process.exit(0);
}
...
try {
  ...
  console.log(`nvim demo: ${actual.toFixed(1)}s recorded, against ${expected.toFixed(1)}s of tape`);
} finally {
