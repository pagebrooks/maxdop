...
jobs:
  maxdop:
    ...
    steps:
      ...
      - run: uvx maxdop@0.1.3 --check src/
```
...
reds every open pull request until somebody reformats the tree. `maxdop@0.1.3` makes that a decision
...
```yaml
      ...
      - name: Check the .sql files this pull request touches
        run: |
          ...
          | uvx maxdop@0.1.3 --check --files-from -
```
...
```yaml
      - run: uvx maxdop@0.1.3 --check --baseline .maxdop-baseline src/
```
...
- **The key carries the version and the platform.** `maxdop-0.1.3-linux-x64` misses the moment you
